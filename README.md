...
- Node.js 18+ (dev server works on 14+, but drizzle-kit requires 16+)
- PostgreSQL
- Redis
...
> If drizzle-kit fails due to Node.js version, apply migrations directly:
> ```bash
> PGPASSWORD=<pass> psql -h localhost -U <user> -d <db> -f lib/db/migrations/<file>.sql
> ```
